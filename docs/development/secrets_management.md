...
- 本番実値は `*.example` に記載しない（テンプレートはダミー値のみ）。

### 2.1 基盤別の責務分界（GitHub/Vercel/Supabase）
- GitHub: CIでのみ必要な秘密値を保持し、ジョブログへ値を出力しない。
- Vercel: 実行環境（Preview/Prod）ごとのアプリ秘密値を分離し、環境取り違えを防ぐ。
- Supabase: DB/Edge等のサーバ用途秘密値を管理し、クライアント公開領域へ露出させない。
...
- 90日を超過する見込みがある場合は、理由・暫定対策・実施予定日を運用記録に残す。
...
### 5.2 即時無効化フロー（運用標準）
1. 漏えい疑いキーの失効を最優先で実施する。
2. 新規キーを発行し、GitHub/Vercel/Supabaseの順で再設定する。
3. デプロイ/実行確認で新規キー反映を検証する。
4. 旧キーが参照不能であることを確認し、運用記録へ残す。
