...
| 種別 | 対象 | ブロック条件 | 対応 |
|------|------|--------------|------|
| PR必須 | `pr-body-traceability` | 失敗/未実行 | PR本文に `対象ID/要件ID/テストID/テスト証跡/セキュリティ確認` を記入 |
...
- シークレット変更時は、90日ローテーション方針と漏えい時即時無効化フローへの影響有無をPR本文に明記する。
...
- PR本文のセキュリティ確認（入力検証/認可/シークレット）が未記載。
- 環境変数 の変更理由と適用環境（Dev/Stg/Prod）が不明。
- シークレット を平文でコミットしている、または秘密管理への移管手順がない。
- シークレット変更PRで、GitHub/Vercel/Supabase の責務分界が説明されていない。
...
- `T-xxx` がPR本文に記載されていない場合は差し戻す。
- 対象ID（`FNC/SCR/IF/TBL`）とテスト証跡リンクが欠落している場合は差し戻す。
- セキュリティ確認（入力検証/認可/シークレット）が未記載の場合は差し戻す。
